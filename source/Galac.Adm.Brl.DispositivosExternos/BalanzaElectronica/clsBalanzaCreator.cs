...
namespace Galac.Adm.Brl.DispositivosExternos.BalanzaElectronica {
    public static class clsBalanzaCreator {
        public static clsBalanza Create(XElement balanzaDat) {
            ...
            case eModeloDeBalanza.CAS_JP_2:
            case eModeloDeBalanza.CAS_PR_II:
                return new clsBalanzaCAS(new clsConexionPuertoSerial(balanzaDat),vModelo);
            default:
                throw new GalacException("Modelo de Balanza aún no implementado",eExceptionManagementType.Controlled);
            }
        }
    }
}
