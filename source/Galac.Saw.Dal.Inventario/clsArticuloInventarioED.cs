...
namespace Galac.Saw.Dal.Inventario {
    ...
    public class clsArticuloInventarioED : LibED, ILibMefDalComponent {
        ...
        private string SqlViewB1() {
            ...
            SQL.AppendLine(" ArticuloInventario.Codigo + ISNULL(ExistenciaPorGrupo.CodigoColor, '') + ISNULL(ExistenciaPorGrupo.CodigoTalla, '') AS CodigoDelArticulo, ");
            SQL.AppendLine(" ArticuloInventario.Descripcion + ISNULL(Color.DescripcionColor, '') + ISNULL(Talla.DescripcionTalla, '') AS Descripcion");
            ...
            SQL.AppendLine(", (CASE WHEN ArticuloInventario.TipoArticuloInv = '1' THEN  (ISNULL(SUM(ExistenciaPorAlmacen.Cantidad), 0) + ISNULL(SUM(ExistenciaPorGrupo.Existencia), 0) + ISNULL(SUM(RenglonExistenciaAlmacen.Cantidad), 0) - SUM(ArticuloInventario.CantArtReservado))");
            SQL.AppendLine("        ELSE  (CASE WHEN  (RenglonExistenciaAlmacen.CodigoSerial IS NULL )  OR  (RenglonExistenciaAlmacen.CodigoRollo IS NULL ) OR  RenglonExistenciaAlmacen.CodigoSerial = '' THEN ISNULL(SUM(ExistenciaPorAlmacen.Cantidad),0)");
            ...
            SQL.AppendLine(" LEFT JOIN ExistenciaPorGrupo ON (ArticuloInventario.Codigo = ExistenciaPorGrupo.CodigoArticulo AND ArticuloInventario.CodigoGrupo = (CASE WHEN ExistenciaPorGrupo.CodigoGrupo = '0' THEN '' ELSE ExistenciaPorGrupo.CodigoGrupo END) AND ArticuloInventario.ConsecutivoCompania = ExistenciaPorGrupo.ConsecutivoCompania )");
            ...
            SQL.AppendLine(" INNER JOIN Gv_EnumTipoDeAlicuota ON ArticuloInventario.AlicuotaIVA COLLATE MODERN_SPANISH_CS_AS = Gv_EnumTipoDeAlicuota.DbValue");
            SQL.AppendLine(" INNER JOIN Gv_EnumTipoArticuloInv ON ArticuloInventario.TipoArticuloInv COLLATE MODERN_SPANISH_CS_AS = Gv_EnumTipoArticuloInv.DbValue");
        ...
        private string SqlViewSearchArticulo() {
            ...
            SQL.AppendLine("    LEFT JOIN Saw.Color ON Saw.Color.ConsecutivoCompania = ExistenciaPorGrupo.ConsecutivoCompania AND Saw.Color.CodigoColor = ExistenciaPorGrupo.CodigoColor");
            SQL.AppendLine("	LEFT JOIN Saw.Talla ON Saw.Talla.ConsecutivoCompania = ExistenciaPorGrupo.ConsecutivoCompania AND Saw.Talla.CodigoTalla = ExistenciaPorGrupo.CodigoTalla");
            SQL.AppendLine("	LEFT JOIN factura INNER JOIN renglonFactura ON (factura.Numero = renglonFactura.NumeroFactura AND factura.ConsecutivoCompania = renglonFactura.ConsecutivoCompania AND factura.ReservarMercancia = 'S' AND factura.StatusFactura = '0' ");
        ...
        bool CrearProcedimientos() {
            ...
            vResult = insSps.CreateStoredProcedure(DbSchema + ".Gp_ObtenerPaginaDeArticulosPorDescripcion", SqlObtenerPaginaDeArticulosParametros(), SqlObtenerPaginaDeArticulosPorDescripcionSentencia(), true) && vResult;
            vResult = insSps.CreateStoredProcedure(DbSchema + ".Gp_ObtenerPaginaDeArticulosPorCodigo", SqlObtenerPaginaDeArticulosParametros(), SqlObtenerPaginaDeArticulosPorCodigoSentencia(), true) && vResult;
