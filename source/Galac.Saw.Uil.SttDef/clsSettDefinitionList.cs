...
namespace Galac.Saw.Uil.SttDef {
    ...
    /// </summary>
    public sealed class clsSettDefinitionList: LibUic, ILibDbSearchModule {
        #region Variables
        SettDefinition _Record;
        ILibBusinessComponentWithSearch<IList<SettDefinition>, IList<SettDefinition>> _Reglas;
        #endregion //Variables
        #region Propiedades
        public SettDefinition Record {
            get { return _Record;}
            set {_Record = value;}
        }
        #endregion //Propiedades
        #region Constructores
        public clsSettDefinitionList() {
            _Record = new SettDefinition();
        }
        ...
        public bool CanBeChoosen(IList<SettDefinition> valRecord, eAccionSR valAction) {
            bool vResult = false;
        ...
        private StringBuilder GetPkParams(XmlDocument valXmlRowDocument) {
            ...
            string vName = insParser.GetString(0, "Name","");
            vParams.AddInString("Name", vName, 50);
            insParser  = null;
            return vParams.Get();
        }
        ...
        void ILibDbSearch.DoAction(object refCallingForm, eAccionSR valAction, string valExtendedAction, XmlDocument valXmlRow) {
            if (valAction == eAccionSR.Escoger) {
                ...
                RegistraCliente();
                IList<SettDefinition> vResulset = _Reglas.GetData(eProcessMessageType.SpName, "SettDefinitionGET", GetPkParams(valXmlRow));
                if (vResulset != null && vResulset.Count > 0) {
                    if (_Reglas.CanBeChoosen(vResulset, valAction)) {
                        clsSettDefinitionIpl vRecord = new clsSettDefinitionIpl();
                        vRecord.ListSettDefinition = vResulset;
                        frmSettDefinitionInput insFrmInput = new frmSettDefinitionInput("Sett Definition", valAction, valExtendedAction);
                        if (insFrmInput.InitLookAndFeelAndSetValues(vRecord.ListSettDefinition, vRecord)) {
        ...
        bool ILibDbSearch.GetData(string valQuery, ref XmlDocument refResulset, StringBuilder valXmlParamsExpression) {
            bool vResult = false;
            RegistraCliente();
            vResult = _Reglas.GetDataFromModule("Sett Definition", ref refResulset, valXmlParamsExpression);
            return vResult;
        }

        LibGalac.Aos.Base.Report.ILibReportInfo ILibDbSearch.GetDataRetrievesInstance() {
            //return new Galac.Saw.Brl.SttDef.Reportes.clsSettDefinitionRpt();
            return null;
        }
        ...
        public static bool ChooseSettDefinition(System.Windows.Window refOwner, ref XmlDocument refXmlDocument, List<LibSearchDefaultValues> valSearchCriteria, List<LibSearchDefaultValues> valFixedCriteria) {
            bool vResult = false;
            vResult = LibFkRetrieval.ChooseRecord(refOwner, new Galac.Saw.Uil.SttDef.Sch.GSSettDefinitionSch(), ref refXmlDocument, valSearchCriteria, valFixedCriteria, "Sett Definition", new clsSettDefinitionList());
            return vResult;
        }
    ...
    } //End of class clsSettDefinitionList
