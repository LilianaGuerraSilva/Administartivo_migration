...
namespace Galac.Adm.Dal.CajaChica {
    public class clsAnticipoDat: LibData, ILibDataComponentWithSearch<IList<Anticipo>, IList<Anticipo>> {
      ...
      //  [PrincipalPermission(SecurityAction.Demand, Role = "Anticipo.Eliminar")]
            ...
            switch (valType) {
                case eProcessMessageType.Message:
                    if (valProcessMessage.Equals("ProximoConsecutivoAnticipo")) {
                        vResult = LibXml.ValueToXElement(insDb.NextLngConsecutive("Adm.Anticipo", "ConsecutivoAnticipo", valParameters), "ConsecutivoAnticipo");
                    }
                    else if (valProcessMessage.Equals("ProximoNumero")) {
                        vResult = LibXml.ValueToXElement(insDb.NextStrConsecutive("Adm.Anticipo", "Numero", valParameters, true, 20), "Numero");
                    }
                    break;
                case eProcessMessageType.SpName:
        ...
        //  ##################3 cambiar  [PrincipalPermission(SecurityAction.Demand, Role = "Anticipo.Modificar")]
        LibResponse ILibDataComponent<IList<Anticipo>, IList<Anticipo>>.Update(IList<Anticipo> refRecord) {
            LibResponse vResult = new LibResponse();
            string vErrMsg ="";
            CurrentRecord = refRecord[0];
            if (ExecuteProcessBeforeUpdate()) {
                eAccionSR accion=eAccionSR.Modificar;
                if (CurrentRecord.StatusAsEnum.Equals(eStatusAnticipo.Anulado))
                    accion = eAccionSR.Anular;
                if (Validate(accion, out vErrMsg)) {
                    LibDatabase insDb = new LibDatabase();
        ...
        protected override bool Validate(eAccionSR valAction, out string outErrorMessage) {
            ...
            vResult = IsValidCodigoBeneficiario(valAction, CurrentRecord.CodigoBeneficiario) && vResult;
            vResult = IsValidCodigoCliente(valAction, CurrentRecord.CodigoCliente) && vResult;
            vResult = IsValidNombreCliente(valAction, CurrentRecord.NombreCliente) && vResult;
            vResult = IsValidCodigoProveedor(valAction, CurrentRecord.CodigoProveedor) && vResult;
            vResult = IsValidNombreProveedor(valAction, CurrentRecord.NombreProveedor) && vResult;
            ...
            vResult = IsValidNombreCuentaBancaria(valAction, CurrentRecord.NombreCuentaBancaria) && vResult;
            vResult = IsValidCodigoConceptoBancario(valAction, CurrentRecord.CodigoConceptoBancario) && vResult;
            vResult = IsValidNombreConceptoBancario(valAction, CurrentRecord.NombreConceptoBancario) && vResult;
        ...
        private bool IsValidConsecutivoAnticipo(eAccionSR valAction, int valConsecutivoCompania, int valConsecutivoAnticipo){
            ...
            if (valConsecutivoAnticipo == 0 && valAction != eAccionSR.Anular) {
                ...
                if (KeyExists(valConsecutivoCompania, vRecBusqueda.ConsecutivoAnticipo)) {
        ...
        private bool IsValidCodigoBeneficiario(eAccionSR valAction, string valCodigoBeneficiario){
            bool vResult = true;
            if ((valAction == eAccionSR.Consultar) || (valAction == eAccionSR.Eliminar)) {
                return true;
            }
            if (_CurrentRecord.TipoAsEnum == eTipoDeAnticipo.Adelanto) {
                valCodigoBeneficiario = LibString.Trim(valCodigoBeneficiario);
                if (LibString.IsNullOrEmpty(valCodigoBeneficiario, true)) {
        ...
        private bool IsValidCodigoCliente(eAccionSR valAction, string valCodigoCliente){
            ...
            if (_CurrentRecord.TipoAsEnum == eTipoDeAnticipo.Cobrado) {
        ...
        private bool IsValidNombreCliente(eAccionSR valAction, string valNombreCliente){
            bool vResult = true;
            if ((valAction == eAccionSR.Consultar) || (valAction == eAccionSR.Eliminar)) {
                return true;
            }
            if (_CurrentRecord.TipoAsEnum == eTipoDeAnticipo.Cobrado) {
                    valNombreCliente = LibString.Trim(valNombreCliente);
                if (LibString.IsNullOrEmpty(valNombreCliente, true)) {
                    BuildValidationInfo(MsgRequiredField("Nombre Cliente"));
                    vResult = false;
                }
            }
            return vResult;
        ...
        private bool IsValidCodigoProveedor(eAccionSR valAction, string valCodigoProveedor){
            ...
            if (_CurrentRecord.TipoAsEnum == eTipoDeAnticipo.Pagado) {
        ...
        private bool IsValidNombreProveedor(eAccionSR valAction, string valNombreProveedor){
            bool vResult = true;
            if ((valAction == eAccionSR.Consultar) || (valAction == eAccionSR.Eliminar)) {
                return true;
            }
        ...
        private bool IsValidNombreCuentaBancaria(eAccionSR valAction, string valNombreCuentaBancaria){
            bool vResult = true;
            if ((valAction == eAccionSR.Consultar) || (valAction == eAccionSR.Eliminar)) {
                return true;
            }
            valNombreCuentaBancaria = LibString.Trim(valNombreCuentaBancaria);
            if (LibString.IsNullOrEmpty(valNombreCuentaBancaria, true)) {
        ...
        private bool IsValidNombreConceptoBancario(eAccionSR valAction, string valNombreConceptoBancario){
            bool vResult = true;
            if ((valAction == eAccionSR.Consultar) || (valAction == eAccionSR.Eliminar)) {
                return true;
            }
            valNombreConceptoBancario = LibString.Trim(valNombreConceptoBancario);
            if (LibString.IsNullOrEmpty(valNombreConceptoBancario, true)) {
                BuildValidationInfo(MsgRequiredField("Nombre Concepto Bancario"));
                vResult = false;
            }
            return vResult;
        }
        ...
        private bool KeyExists(int valConsecutivoCompania, int valConsecutivoAnticipo) {
            ...
            LibDatabase insDb = new LibDatabase();
            vResult = insDb.ExistsRecord("Adm.Anticipo", "ConsecutivoCompania", ParametrosClave(vRecordBusqueda, false, false));
            insDb.Dispose();
            return vResult;
        }
        ...
        private bool KeyExists(int valConsecutivoCompania, Anticipo valRecordBusqueda) {
            ...
            vResult = insDb.ExistsRecord("Adm.Anticipo", "ConsecutivoCompania", ParametrosClave(valRecordBusqueda, false, false));
            insDb.Dispose();
            return vResult;
        }
