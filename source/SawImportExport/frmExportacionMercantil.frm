...
Begin VB.Form frmExportacionMercantil 
   BackColor       =   &H00F3F3F3&
   ...
   End
End
Attribute VB_Name = "frmExportacionMercantil"
...
Private Function CM_FILE_NAME() As String
   CM_FILE_NAME = "frmExportacionMercantil"
End Function
...
Private Sub cmdGrabar_Click()
   ...
   If fVerificaQueDeseaExportarElArchivoALaRutaEspecificada() Then
      ...
      sGenerarArchivoDeExportacionParaMercantil
...
Private Sub sGenerarArchivoDeExportacionParaMercantil()
   Dim varSqlEncabezado As String
   Dim insExportar As clsExportarImportar
   On Error GoTo h_ERROR
   Set insExportar = New clsExportarImportar
   
   prgBarExportarDatos.Visible = True
   varSqlEncabezado = insExportar.fSQLExportarCabeceraMercantil(txtCodigoCuentaBancaria, dtpFechaInicial.Value, dtpFechaFinal.Value, dtpFechaGestion.Value)
   If fCreaElArchivoDeExportacion(txtNombreDelArchivo.Text) Then
      If fExportarArchMercantil(varSqlEncabezado, txtNombreDelArchivo.Text) Then
...
h_EXIT: On Error GoTo 0
   Exit Sub
h_ERROR: Err.Raise Err.Number, Err.Source, gError.fAddMethodToStackTrace(Err.Description, CM_FILE_NAME, "sGenerarArchivoDeExportacionParaMercantil", CM_MESSAGE_NAME, GetGender(), Err.HelpContext, Err.HelpFile, Err.LastDllError)
End Sub
...
Public Function fExportarArchMercantil(ByVal valSqlEncabezado As String, ByVal valNombreDeArchivo As String) As Boolean
   Dim Rs As ADODB.Recordset
   Dim linea As String
   On Error GoTo h_ERROR
   Set Rs = New ADODB.Recordset
   'Cabecera de Arch Mercantil
   If gDbUtil.fOpenRecordSetAllParameters(Rs, valSqlEncabezado, Conexion, adLockOptimistic, adUseClient, adOpenStatic) Then
      If Rs.RecordCount > 0 Then
      
         linea = Rs.Fields("Cabecera") & _
         Rs.Fields("IdBanco") & _
         Rs.Fields("NumeroArchivo") & _
         Rs.Fields("TipoProducto") & _
         Rs.Fields("TipoPago") & _
         Rs.Fields("TipoId") & _
         Rs.Fields("Rif") & _
         Rs.Fields("TotalRegistro") & _
         Rs.Fields("TotalDocumentos") & _
         Rs.Fields("FechaGestion") & _
         Rs.Fields("NumeroCuenta") & _
         Rs.Fields("AreaReservada1") & _
         Rs.Fields("NumeroSerial") & _
         Rs.Fields("CodigoRespuesta") & _
         Rs.Fields("FechaProceso") & _
         Rs.Fields("AreaReservada2")
         
         gUtilFile.fGrabarDatosEnElArchivo valNombreDeArchivo, linea
         
         fExportarArchPagoMercantil (valNombreDeArchivo)
         gApi.sAddProgress prgBarExportarDatos
      End If
   End If
   fExportarArchMercantil = True
   gDbUtil.sDestroyRecordSet Rs
h_EXIT: On Error GoTo 0
   Exit Function
h_ERROR: Err.Raise Err.Number, Err.Source, gError.fAddMethodToStackTrace(Err.Description, CM_FILE_NAME, "fExportarArchMercantil", CM_MESSAGE_NAME, GetGender(), Err.HelpContext, Err.HelpFile, Err.LastDllError)
End Function

Public Function fExportarArchPagoMercantil(ByVal valNombreDeArchivo As String) As Boolean
   Dim Rs As ADODB.Recordset
   Dim linea As String
   Dim varSqlPago As String
   Dim insExportar As clsExportarImportar
   ...
   Set Rs = New ADODB.Recordset
   Set insExportar = New clsExportarImportar
   'Pago de Arch Mercantil
   varSqlPago = insExportar.fSQLExportarPagoMercantil(txtCodigoCuentaBancaria, dtpFechaInicial.Value, dtpFechaFinal.Value, OptTipoPago(0).Value)
   If gDbUtil.fOpenRecordSetAllParameters(Rs, varSqlPago, Conexion, adLockOptimistic, adUseClient, adOpenStatic) Then
      If Rs.RecordCount > 0 Then
         Rs.MoveFirst
         For contRegistro = 0 To Rs.RecordCount - 1
            linea = Rs.Fields("detalle") & _
            Rs.Fields("TipoId") & _
            Rs.Fields("Rif") & _
            Rs.Fields("FormaPago") & _
            Rs.Fields("AreaReservada1") & _
            Rs.Fields("AreaReservada2") & _
            Rs.Fields("AreaReservada3") & _
            Rs.Fields("NumeroCuentaProv") & _
            Rs.Fields("TotalDocumentos") & _
            Rs.Fields("idCliente") & _
            Rs.Fields("TipoPago") & _
            Rs.Fields("AreaReservada4") & _
            Rs.Fields("Proveedor") & _
            Rs.Fields("AreaReservada5") & _
            Rs.Fields("NumeroComprobante") & _
            Rs.Fields("email") & _
            Rs.Fields("CodRespuesta") & _
            Rs.Fields("MsjRespuesta") & _
            Rs.Fields("ConceptoPago") & _
            Rs.Fields("AreaReservada6")
             
            gUtilFile.fGrabarDatosEnElArchivo valNombreDeArchivo, linea
            
            vNumeroComprobante = Rs.Fields("NumeroComprobante")
            fExportarArchOperacionesMercantil valNombreDeArchivo, vNumeroComprobante
   ...
   End If
   Set insExportar = Nothing
   fExportarArchPagoMercantil = True
   gDbUtil.sDestroyRecordSet Rs
h_EXIT: On Error GoTo 0
   Exit Function
h_ERROR: Err.Raise Err.Number, Err.Source, gError.fAddMethodToStackTrace(Err.Description, CM_FILE_NAME, "fExportarArchPagoMercantil", CM_MESSAGE_NAME, GetGender(), Err.HelpContext, Err.HelpFile, Err.LastDllError)
End Function

Public Function fExportarArchOperacionesMercantil(ByVal valNombreDeArchivo As String, ByVal valNumeroComprobante As String) As Boolean
   Dim Rs As ADODB.Recordset
   Dim linea As String
   Dim varSqlOperaciones As String
   Dim insExportar As clsExportarImportar
   Dim contRegistro As Integer
   On Error GoTo h_ERROR
   Set Rs = New ADODB.Recordset
   Set insExportar = New clsExportarImportar
   'Pago de Arch Mercantil
   varSqlOperaciones = insExportar.fSQLExportarOperacionMercantil(txtCodigoCuentaBancaria, dtpFechaInicial.Value, dtpFechaFinal.Value, OptTipoPago(0).Value, valNumeroComprobante)
   
   If gDbUtil.fOpenRecordSetAllParameters(Rs, varSqlOperaciones, Conexion, adLockOptimistic, adUseClient, adOpenStatic) Then
      If Rs.RecordCount > 0 Then
         Rs.MoveFirst
         For contRegistro = 0 To Rs.RecordCount - 1
            linea = Rs.Fields("Operacion") & _
            Rs.Fields("NumeroCheque") & _
            Rs.Fields("TipoDocumento") & _
            Rs.Fields("TotalDocumento") & _
            Rs.Fields("fecha") & _
            Rs.Fields("AreaReservada1") & _
            Rs.Fields("NumeroComprobante") & _
            Rs.Fields("TotalDocumento") & _
            Rs.Fields("NumeroOrden") & _
            Rs.Fields("DescripcionPago") & _
            Rs.Fields("AreaReservada2")
   ...
   End If
   Set insExportar = Nothing
   fExportarArchOperacionesMercantil = True
   gDbUtil.sDestroyRecordSet Rs
h_EXIT: On Error GoTo 0
   Exit Function
h_ERROR: Err.Raise Err.Number, Err.Source, gError.fAddMethodToStackTrace(Err.Description, CM_FILE_NAME, "fExportarArchOperacionesMercantil", CM_MESSAGE_NAME, GetGender(), Err.HelpContext, Err.HelpFile, Err.LastDllError)
End Function
