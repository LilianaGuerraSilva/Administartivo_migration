...
namespace Galac.Saw.Uil.Inventario {
    ...
    /// </summary>
    public sealed class clsTallaList: LibUicMF, ILibDbSearchModule {
        #region Variables
        Talla _Record;
        ...
        public clsTallaList(LibXmlMemInfo initAppMemoryInfo, LibXmlMFC initMFC):base(initAppMemoryInfo, initMFC) {
            _Record = new Talla();
        }
        ...
        public bool CanBeChoosen(IList<Talla> valRecord, eAccionSR valAction) {
            bool vResult = false;
        ...
        private StringBuilder GetPkParams(XmlDocument valXmlRowDocument) {
            ...
            string vCodigoTalla = insParser.GetString(0, "CodigoTalla","");
            vParams.AddInString("CodigoTalla", vCodigoTalla, 3);
            insParser  = null;
            return vParams.Get();
        }
        ...
        void ILibDbSearch.DoAction(object refCallingForm, eAccionSR valAction, string valExtendedAction, XmlDocument valXmlRow) {
            if (valAction == eAccionSR.Escoger) {
                ...
                RegistraCliente();
                IList<Talla> vResulset = _Reglas.GetData(eProcessMessageType.SpName, "TallaGET", GetPkParams(valXmlRow));
                if (vResulset != null && vResulset.Count > 0) {
                    if (_Reglas.CanBeChoosen(vResulset, valAction)) {
                        clsTallaIpl vRecord = new clsTallaIpl(AppMemoryInfo, Mfc);
                        vRecord.ListTalla = vResulset;
                        frmTallaInput insFrmInput = new frmTallaInput("Talla", valAction, valExtendedAction);
                        if (insFrmInput.InitLookAndFeelAndSetValues(vRecord.ListTalla, vRecord)) {
        ...
        bool ILibDbSearch.GetData(string valQuery, ref XmlDocument refResulset, StringBuilder valXmlParamsExpression) {
            bool vResult = false;
            RegistraCliente();
            vResult = _Reglas.GetDataFromModule("Talla", ref refResulset, valXmlParamsExpression);
            return vResult;
        }
        ...
        LibGalac.Aos.Base.Report.ILibReportInfo ILibDbSearch.GetDataRetrievesInstance() {
            ...
            return new Galac.Saw.Brl.Inventario.Reportes.clsTallaRpt(Sinonimo);
        }
        ...
        public static bool ChooseTalla(System.Windows.Window refOwner, ref XmlDocument refXmlDocument, List<LibSearchDefaultValues> valSearchCriteria, List<LibSearchDefaultValues> valFixedCriteria) {
            bool vResult = false;
            vResult = LibFkRetrieval.ChooseRecord(refOwner, new Galac.Saw.Uil.Inventario.Sch.GSTallaSch(), ref refXmlDocument, valSearchCriteria, valFixedCriteria, "Talla", new clsTallaList(null, null));
            return vResult;
        }
