...
namespace Galac.Saw.Uil.Inventario {
    ...
    /// </summary>
    public sealed class clsCategoriaList: LibUicMF, ILibDbSearchModule {
        #region Variables
        Categoria _Record;
        ILibBusinessComponentWithSearch<IList<Categoria>, IList<Categoria>> _Reglas;
        #endregion //Variables
        #region Propiedades
        public Categoria Record {
            get { return _Record;}
            set {_Record = value;}
        }
        #endregion //Propiedades
        #region Constructores
        public clsCategoriaList(LibXmlMemInfo initAppMemoryInfo, LibXmlMFC initMFC):base(initAppMemoryInfo, initMFC) {
            _Record = new Categoria();
        }
        ...
        public bool CanBeChoosen(IList<Categoria> valRecord, eAccionSR valAction) {
            bool vResult = false;
        ...
        void ILibDbSearch.DoAction(object refCallingForm, eAccionSR valAction, string valExtendedAction, XmlDocument valXmlRow) {
            if (valAction == eAccionSR.Escoger) {
                ...
                RegistraCliente();
                IList<Categoria> vResulset = _Reglas.GetData(eProcessMessageType.SpName, "CategoriaGET", GetPkParams(valXmlRow));
                if (vResulset != null && vResulset.Count > 0) {
                    if (_Reglas.CanBeChoosen(vResulset, valAction)) {
                        clsCategoriaIpl vRecord = new clsCategoriaIpl(AppMemoryInfo, Mfc);
                        vRecord.ListCategoria = vResulset;
                        frmCategoriaInput insFrmInput = new frmCategoriaInput("Categoria", valAction, valExtendedAction);
                        if (insFrmInput.InitLookAndFeelAndSetValues(vRecord.ListCategoria, vRecord)) {
        ...
        bool ILibDbSearch.GetData(string valQuery, ref XmlDocument refResulset, StringBuilder valXmlParamsExpression) {
            bool vResult = false;
            RegistraCliente();
            vResult = _Reglas.GetDataFromModule("Categoria", ref refResulset, valXmlParamsExpression);
            return vResult;
        }

        LibGalac.Aos.Base.Report.ILibReportInfo ILibDbSearch.GetDataRetrievesInstance() {
            return new Galac.Saw.Brl.Inventario.Reportes.clsCategoriaRpt();
        }
        ...
        public static bool ChooseCategoria(System.Windows.Window refOwner, ref XmlDocument refXmlDocument, List<LibSearchDefaultValues> valSearchCriteria, List<LibSearchDefaultValues> valFixedCriteria) {
            bool vResult = false;
            vResult = LibFkRetrieval.ChooseRecord(refOwner, new Galac.Saw.Uil.Inventario.Sch.GSCategoriaSch(), ref refXmlDocument, valSearchCriteria, valFixedCriteria, "Categoria", new clsCategoriaList(null, null));
            return vResult;
        }
