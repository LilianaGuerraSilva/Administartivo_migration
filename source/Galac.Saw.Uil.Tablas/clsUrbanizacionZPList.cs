...
namespace Galac.Saw.Uil.Tablas {
    ...
    /// </summary>
    public sealed class clsUrbanizacionZPList: LibUic, ILibDbSearchModule {
        #region Variables
        UrbanizacionZP _Record;
        ILibBusinessComponentWithSearch<IList<UrbanizacionZP>, IList<UrbanizacionZP>> _Reglas;
        #endregion //Variables
        #region Propiedades
        public UrbanizacionZP Record {
            get { return _Record;}
            set {_Record = value;}
        }
        #endregion //Propiedades
        #region Constructores
        public clsUrbanizacionZPList() {
            _Record = new UrbanizacionZP();
        }
        ...
        public bool CanBeChoosen(IList<UrbanizacionZP> valRecord, eAccionSR valAction) {
            bool vResult = false;
        ...
        private StringBuilder GetPkParams(XmlDocument valXmlRowDocument) {
            ...
            string vUrbanizacion = insParser.GetString(0, "Urbanizacion","");
            vParams.AddInString("Urbanizacion", vUrbanizacion, 30);
            insParser  = null;
            return vParams.Get();
        }
        ...
        void ILibDbSearch.DoAction(object refCallingForm, eAccionSR valAction, string valExtendedAction, XmlDocument valXmlRow) {
            if (valAction == eAccionSR.Escoger) {
                ...
                RegistraCliente();
                IList<UrbanizacionZP> vResulset = _Reglas.GetData(eProcessMessageType.SpName, "UrbanizacionZPGET", GetPkParams(valXmlRow));
                if (vResulset != null && vResulset.Count > 0) {
                    if (_Reglas.CanBeChoosen(vResulset, valAction)) {
                        clsUrbanizacionZPIpl vRecord = new clsUrbanizacionZPIpl();
                        vRecord.ListUrbanizacionZP = vResulset;
                        frmUrbanizacionZPInput insFrmInput = new frmUrbanizacionZPInput("Urbanización - Zona Postal", valAction, valExtendedAction);
                        if (insFrmInput.InitLookAndFeelAndSetValues(vRecord.ListUrbanizacionZP, vRecord)) {
        ...
        bool ILibDbSearch.GetData(string valQuery, ref XmlDocument refResulset, StringBuilder valXmlParamsExpression) {
            bool vResult = false;
            RegistraCliente();
            vResult = _Reglas.GetDataFromModule("Urbanización - Zona Postal", ref refResulset, valXmlParamsExpression);
            return vResult;
        }

        LibGalac.Aos.Base.Report.ILibReportInfo ILibDbSearch.GetDataRetrievesInstance() {
            return new Galac.Saw.Brl.Tablas.Reportes.clsUrbanizacionZPRpt();
        }
        ...
        public static bool ChooseUrbanizacionZP(System.Windows.Window refOwner, ref XmlDocument refXmlDocument, List<LibSearchDefaultValues> valSearchCriteria, List<LibSearchDefaultValues> valFixedCriteria) {
            bool vResult = false;
            vResult = LibFkRetrieval.ChooseRecord(refOwner, new Galac.Saw.Uil.Tablas.Sch.GSUrbanizacionZPSch(), ref refXmlDocument, valSearchCriteria, valFixedCriteria, "Urbanización - Zona Postal", new clsUrbanizacionZPList());
            return vResult;
        }
    ...
    } //End of class clsUrbanizacionZPList
