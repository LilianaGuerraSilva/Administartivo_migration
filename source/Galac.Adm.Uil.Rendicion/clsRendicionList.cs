...
namespace Galac.Adm.Uil.CajaChica {
    ...
    /// </summary>
    public sealed class clsRendicionList: LibUicMF, ILibDbSearchModule {
        #region Variables
        Rendicion _Record;
        ILibBusinessMasterComponentWithSearch<IList<Rendicion>, IList<Rendicion>> _Reglas;
        frmRendicionInput insFrmInput;
        string _Result = "";
        #endregion //Variables
        #region Propiedades
        ...
        public clsRendicionList(LibXmlMemInfo initAppMemoryInfo, LibXmlMFC initMFC):base(initAppMemoryInfo, initMFC) {
            ...
            if (valAction == eAccionSR.Escoger){
                ...
                if (vResulset != null && vResulset.Count > 0) {
                    if (_Reglas.CanBeChoosen(vResulset, valAction)) {
                        clsRendicionIpl vRecord = new clsRendicionIpl(AppMemoryInfo, Mfc);
                        vRecord.ListRendicion = vResulset;
                        insFrmInput = new frmRendicionInput("Rendicion", valAction, valExtendedAction);
        ...
        LibGalac.Aos.Base.Report.ILibReportInfo ILibDbSearch.GetDataRetrievesInstance() {
            return null; //new Galac.Adm.Brl.CajaChica.Reportes.clsRendicionRpt();
        }
        ...
        public static bool ChooseRendicion(System.Windows.Window refOwner, ref XmlDocument refXmlDocument, List<LibSearchDefaultValues> valSearchCriteria, List<LibSearchDefaultValues> valFixedCriteria) {
            bool vResult = false;
            vResult = LibFkRetrieval.ChooseRecord(refOwner, new Galac.Adm.Uil.CajaChica.Sch.GSRendicionSch(), ref refXmlDocument, valSearchCriteria, valFixedCriteria, "Rendicion", new clsRendicionList(null, null));
            return vResult;
        }
