...
namespace Galac.Adm.Uil.CajaChica {
    public class clsReposicionIpl: LibMROMF, ILibView { 
        #region Variables
        ...
        public clsReposicionIpl(LibXmlMemInfo initAppMemoryInfo, LibXmlMFC initMfc)
     ...
     private void RegistraCliente() {
        ...
        object ILibView.NextSequential(string valSequentialName) {
            return GenerarSiguiente(valSequentialName); 
        }
        ...
        bool ILibView.SpecializedUpdateRecord(object refRecord, string valAction, out string outErrorMsg) {
            ...
            else if (valAction.Equals(eAccionSR.Contabilizar.ToString()) || valAction.Equals(eAccionSR.ReImprimir.ToString()))
                return GenerarInfoContab(((Rendicion)refRecord), eAccionSR.Anular, out outErrorMsg).Success;
            else
                return false;
       ...
       [PrincipalPermission(SecurityAction.Demand, Role = "Reposicion de Caja Chica.Insertar")]
        ...
        public bool ValidateAll(Rendicion refRecord, eAccionSR valAction, out string outErrorMessage) {
         ...
         //   vResult = IsValidConsecutivoBeneficiario(valAction, refRecord.ConsecutivoBeneficiario, false) && vResult;
            ...
            if (valAction.Equals(eAccionSR.Cerrar)) {
                vResult = EsValidaCuentaBancariaCajaChica(refRecord.ConsecutivoCompania,refRecord.CodigoCtaBancariaCajaChica) && vResult;
                ...
                vResult = IsValidBeneficiarioCheque(valAction, refRecord.BeneficiarioCheque, false) && vResult;
			     ...
			     respuesta = ((Galac.Adm.Brl.CajaChica.clsRendicionNav)_Reglas).verificaCXPSonValidas(refRecord);
                if (!respuesta.Success) {
                    parseLibResponse(respuesta, ref outErrorMessage);
            ...
            clsDetalleDeRendicionIpl detalleIpl = new clsDetalleDeRendicionIpl(initAppMemoryInfo, initMfc);

            var Hayduplicados = false;
            foreach (var item in refRecord.DetailDetalleDeRendicion)
	        {

                int total = ((refRecord.DetailDetalleDeRendicion).Where<DetalleDeRendicion>(de => de.NumeroDocumento == item.NumeroDocumento && de.CodigoProveedor == item.CodigoProveedor).Count());
                if (total > 1)
                {
                    Hayduplicados = true;
                    item.ValidoAsBool = false;
                    item.ErrorMsj = "Este Registro se encuentra Repetido.";
                    vResult = false;
                }

                vResult = vResult &&  detalleIpl.ValidateAll(item,valAction,out outErrorMessage); 
	        }

            if(Hayduplicados)
                Information.Append("Algunos registros se encuentran repetidos.\n");

            outErrorMessage += Information.ToString() + " " + detalleIpl.Information.ToString();
            return vResult;
        }
        ...
        public bool IsValidFechaCierre(eAccionSR valAction, DateTime valFechaCierre, bool valCleanInfoBeforeStart){
            ...
            if (valFechaCierre>DateTime.Today)
            {
                BuildValidationInfo("No se puede cerrar una reposición para una fecha posterior al día de hoy.");
                vResult = false;
            }
        ...
        public bool IsValidFechaAnulacion(eAccionSR valAction, DateTime valFechaAnulacion, bool valCleanInfoBeforeStart){
            ...
            if (valFechaAnulacion > DateTime.Today)
            {
                BuildValidationInfo("No se puede anular una reposición para una fecha posterior al día de hoy.");
                vResult = false;
            }
        ...
        bool EsValidaCuentaBancariaCajaChica(int valConsecutivoCompania, string valCtaBancariaCajaChica) {
            bool vResult = false;
            if (((IRendicionPdn)_Reglas).EsValidaCuentaBancariaCajaChica(valConsecutivoCompania, valCtaBancariaCajaChica)) {
                vResult = true;
            } else {
                BuildValidationInfo("La cuenta bancaria seleccionada para el Cierre de Reposición Caja Chica, ya no es de tipo Caja Chica.");
                vResult = false;
            }
            return vResult;
        }
        ...
        private string GenerarSiguiente(string val)
        {
            string vResult = "";
            RegistraCliente();
            XElement vResulset = (_Reglas).QueryInfo(eProcessMessageType.Message, val, Mfc.GetIntAsParam("Compania"), true);
            vResult = LibXml.GetPropertyString(vResulset, val);
            return vResult;
        }
        private LibResponse GenerarInfoContab(Rendicion rendicion, eAccionSR eAccionSR, out string outErrorMsg)
        {
            LibResponse vResult;
            outErrorMsg = "";
            RegistraCliente();
            vResult = ((IRendicionPdn)_Reglas).GenerarInfoContab(rendicion);
            if (vResult.Success)
            {
                if (vResult.Info != null)
                {
                    ...
                    {
                        if (vResult.Info.Name == "DatosContab")
        ...
        internal LibResponse cerrar(Rendicion refRecord, eAccionSR valAction, out string outErrorMessage)
        {
            ...
            int consecutivo = initAppMemoryInfo.GlobalValuesGetInt("DatosDocumento", "Consecutivo");
            RegistraCliente();
            if (ValidateAll(refRecord, valAction, out outErrorMessage))
            {
                if (consecutivo < 0)
                {
                    vResult.Success = UpdateRecord(refRecord, valAction, out outErrorMessage);
                    vResult.Info = ((IRendicionPdn)_Reglas).generarResultadoXml(refRecord);
                    parseLibResponse(vResult, ref outErrorMessage);
                    return vResult;
                }
                vResult = ((IRendicionPdn)_Reglas).cerrarReposicion(refRecord);
            }
            else
            {
        ...
        internal LibResponse Anular(Rendicion refRecord, eAccionSR valAction, out string outErrorMessage)
        {
            ...
            if (ValidateAll(refRecord, valAction, out outErrorMessage))
            {
                ...
                vResult = ((IRendicionPdn)_Reglas).anularReposicion(refRecord);
                if (vResult.Success)
                    LibGalac.Aos.UI.Wpf.LibNotifier.Information(null, "Se ha anulado el Registro correctamente.", "Anulación");
            }
            else
            {
                vResult.Success = false;
        ...
        private void parseLibResponse(LibResponse vResult, ref string outErrorMessage)
        {
            if (vResult.Info != null)
            {
                vResult.Info.MoveToContent();
                ...
                while (!vResult.Info.EOF)
                {
                    if (vResult.Info.Name == "Error")
                    {
                        //outErrorMessage = 
                        XDocument xd = XDocument.Parse(vResult.Info.ReadOuterXml());
                        foreach (XElement e in xd.Descendants())
                        {
                            if (e.Name == "ErrorCode")
                                ResultadoOperacion = e.Value;
                            else if (e.Name == "Mensaje")
                                outErrorMessage = e.Value;
                        }
                    }
                    else if (vResult.Info.Name == "DatosContab")
                        ResultadoOperacion = vResult.Info.ReadInnerXml();
                    else if (vResult.Info.Name == "ErrorCode")
                        ResultadoOperacion = vResult.Info.ReadElementContentAsString();
                    vResult.Info.Read();
                }
            }
        }
