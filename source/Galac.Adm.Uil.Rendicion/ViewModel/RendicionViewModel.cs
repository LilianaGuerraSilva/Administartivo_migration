...
namespace Galac.Adm.Uil.CajaChica.ViewModel {
    public class RendicionViewModel : LibInputMasterViewModelMfc<Rendicion> {
        ...
        private void ExecuteChooseCodigoCtaBancariaCajaChicaCommand(string valCodigo) {
            try {
                ...
                if (ConexionCodigoCtaBancariaCajaChica != null) {
                    if (ValidarTipoDeContribuyenteIGTF(ConexionCodigoCtaBancariaCajaChica, Action)) {
        ...
        private void ExecuteChooseCodigoCuentaBancariaCommand(string valCodigo) {
            try {
                ...
                if (ConexionCodigoCuentaBancaria != null) {
                    if (ValidarTipoDeContribuyenteIGTF(ConexionCodigoCuentaBancaria, Action)) {                        
        ...
        private void OnCodigoCuentaBancariaChanged() {
            PuedoGenerarITF = ConexionCodigoCuentaBancaria.GeneraMovBancarioPorIGTF && ConexionCodigoCuentaBancaria.ManejaDebitoBancario && LibGlobalValues.Instance.GetAppMemInfo().GlobalValuesGetBool("Parametros", "ManejaDebitoBancario");
            GeneraImpuestoBancario = PuedoGenerarITF;
        }

        private bool ValidarTipoDeContribuyenteIGTF(FkCuentaBancariaViewModel vConexionCuentaBancaria, eAccionSR valAction) {
            bool vResult = false;
            bool vManejaDebitoBancario = LibGlobalValues.Instance.GetAppMemInfo().GlobalValuesGetBool("Parametros", "ManejaDebitoBancario");
            bool vManejaCreditoBancario = LibGlobalValues.Instance.GetAppMemInfo().GlobalValuesGetBool("Parametros", "ManejaCreditoBancario");
            if ((valAction == eAccionSR.Insertar || valAction == eAccionSR.Modificar || valAction == eAccionSR.Cerrar) && (vManejaDebitoBancario || vManejaCreditoBancario) && (vConexionCuentaBancaria.ManejaDebitoBancario || vConexionCuentaBancaria.ManejaCreditoBancario) && (vConexionCuentaBancaria.TipoDeAlicuotaPorContribuyente == eTipoAlicPorContIGTF.NoAsignado)) {
                LibMessages.MessageBox.Alert(this, "La cuenta bancaria que ha seleccionado (" + vConexionCuentaBancaria.NombreCuenta + ") genera I.G.T.F., pero no tiene un Tipo de Alícuota por Contribuyente asignado. Debe modificar esta cuenta bancaria antes de continuar.", ModuleName);
                vResult = false;
            } else {
                vResult = true;
            }
