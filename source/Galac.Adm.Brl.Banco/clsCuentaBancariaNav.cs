...
namespace Galac.Adm.Brl.Banco {
    public partial class clsCuentaBancariaNav: LibBaseNav<IList<CuentaBancaria>, IList<CuentaBancaria>>, ICuentaBancariaPdn {
        ...
        System.Xml.Linq.XElement GetPametrosAdministrativoCodigoMonedaLocal(int valConsecutivoCompania, ILibDataComponent<IList<CuentaBancaria>, IList<CuentaBancaria>> instanciaDal) {
            StringBuilder sql;
            string vDbSchema = "";
            if (ExisteTablaSettValueByCompany()) {
                if(LibDefGen.ProgramInfo.IsCountryEcuador()) {
        ...
        System.Xml.Linq.XElement GetPametrosAdministrativoNombreMonedaLocal(int valConsecutivoCompania, ILibDataComponent<IList<CuentaBancaria>, IList<CuentaBancaria>> instanciaDal) {
            StringBuilder sql;
            string vDbSchema = "";
            if (ExisteTablaSettValueByCompany()) {
                if(LibDefGen.ProgramInfo.IsCountryEcuador()) {
        ...
        bool ExisteCuentaBancaria(int valConsecutivoCompania, string valCodigoCuenta) {
            Galac.Adm.Dal.Banco.clsCuentaBancariaDat insDal = new Galac.Adm.Dal.Banco.clsCuentaBancariaDat();
        ...
        bool ICuentaBancariaPdn.ActualizaSaldoDisponibleEnCuenta(int valConsecutivoCompania, string valCodigoCuenta, string valMonto, string valIngresoEgreso, int valmAction, string valMontoOriginal, bool valSeModificoTipoConcepto) {
            bool vResult;
            vResult = false;
            vResult = new Galac.Adm.Dal.Banco.clsCuentaBancariaDat().ActualizaSaldoDisponibleEnCuenta(valConsecutivoCompania, valCodigoCuenta, valMonto, valIngresoEgreso, valmAction, valMontoOriginal, valSeModificoTipoConcepto);
            return vResult;
        }
        ...
        bool ICuentaBancariaPdn.RecalculaSaldoCuentasBancarias(int valConsecutivoCompania) {
            bool vResult;
            vResult = false;
            vResult = new Galac.Adm.Dal.Banco.clsCuentaBancariaDat().RecalculaSaldoCuentasBancarias(valConsecutivoCompania);
            return vResult;
        }
        ...
        bool ICuentaBancariaPdn.TieneMovimientosBancariosDeReposicionCajaChica(int valConsecutivoCompania, string valCodigoCuentaBancaria) {
            int vResult;
            XElement vResultset;
            vResultset = new Galac.Adm.Dal.Banco.clsCuentaBancariaDat().MovimientosBancariosDeReposicionCajaChica(valConsecutivoCompania, valCodigoCuentaBancaria);
            if (vResultset != null) {
        ...
        bool ICuentaBancariaPdn.EsValidaCuentaBancariaCajaChica(int valConsecutivoCompania, string valCodigoCuentaBancaria) {
            int vResult;
            XElement vResultset;
            vResultset = new Galac.Adm.Dal.Banco.clsCuentaBancariaDat().EsValidaCuentaBancariaCajaChica(valConsecutivoCompania, valCodigoCuentaBancaria);
            if (vResultset != null) {
                vResult = (from vRecord in vResultset.Descendants("GpResult")
                           select vRecord).Count();
            } else {
        ...
        bool ICuentaBancariaPdn.ExistenMovimientosCuentaBancaria(int valConsecutivoCompania) {
           int vResult;
           XElement vResultset;
           vResultset = new Galac.Adm.Dal.Banco.clsCuentaBancariaDat().MovimientosBancariosPorCuentaBancaria(valConsecutivoCompania);
           if (vResultset != null) {
              vResult = (from vRecord in vResultset.Descendants("GpResult")
                         select vRecord).Count();
           } else {
