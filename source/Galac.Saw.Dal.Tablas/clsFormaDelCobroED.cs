...
namespace Galac.Saw.Dal.Tablas {
    ...
    public class clsFormaDelCobroED: LibED, ILibMefDalComponent {
        ...
        private string SqlCreateTable() {
            ...
            SQL.AppendLine("TipoDePago" + InsSql.CharTypeForDb(1) + " CONSTRAINT d_ForDelCobTiDePa DEFAULT ('0'), ");
            SQL.AppendLine("fldTimeStamp" + InsSql.TimeStampTypeForDb() + ",");
            SQL.AppendLine("CONSTRAINT p_FormaDelCobro PRIMARY KEY CLUSTERED");
            SQL.AppendLine("(Codigo ASC)");
            SQL.AppendLine(")");
            return SQL.ToString();
        }
        ...
        private string SqlViewB1() {
            StringBuilder SQL = new StringBuilder();
            SQL.AppendLine("SELECT Codigo, Nombre, TipoDePago, " + DbSchema + ".Gv_EnumTipoDeFormaDePago.StrValue AS TipoDePagoStr");
        ...
        private string SqlSpUpd() {
            ...
            SQL.AppendLine("   IF EXISTS(SELECT Codigo FROM " + DbSchema + ".FormaDelCobro WHERE Codigo = @Codigo)");
            SQL.AppendLine("   BEGIN");
            SQL.AppendLine("      SELECT @CurrentTimeStamp = fldTimeStamp FROM " + DbSchema + ".FormaDelCobro WHERE Codigo = @Codigo");
            SQL.AppendLine("      IF (CAST(@CurrentTimeStamp AS bigint) = @TimeStampAsInt)");
            SQL.AppendLine("      BEGIN");
            SQL.AppendLine("--Para Validaciones de FK Lógicas crear e invocar:DECLARE @CanBeChanged bit; EXEC @CanBeChanged = " + DbSchema + ".Gp_FormaDelCobroCanBeUpdated @Codigo, @CurrentTimeStamp, @ValidationMsg out");
        ...
        private string SqlSpDel() {
            ...
            SQL.AppendLine("      IF (CAST(@CurrentTimeStamp AS bigint) = @TimeStampAsInt)");
            SQL.AppendLine("      BEGIN");
            SQL.AppendLine("--Para Validaciones de FK Lógicas crear e invocar:DECLARE @CanBeDeleted bit; EXEC @CanBeDeleted = " + DbSchema + ".Gp_FormaDelCobroCanBeDeleted @Codigo, @CurrentTimeStamp, @ValidationMsg out");
        ...
        private string SqlSpGet() {
            ...
            SQL.AppendLine("      FROM " + DbSchema + ".FormaDelCobro");
            SQL.AppendLine("      WHERE FormaDelCobro.Codigo = @Codigo");
        ...
        private string SqlSpGetFK() {
            ...
            SQL.AppendLine("      " + DbSchema + ".FormaDelCobro.TipoDePago");
        ...
        bool CrearVistas(){
            ...
            vResult = insVistas.Create(DbSchema + ".Gv_EnumTipoDeFormaDePago", LibTpvCreator.SqlViewStandardEnum(typeof(eTipoDeFormaDePago), InsSql), true, true);
