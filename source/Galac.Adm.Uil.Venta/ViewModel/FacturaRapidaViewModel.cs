...
namespace Galac.Adm.Uil.Venta.ViewModel {
    public class FacturaRapidaViewModel : LibInputMasterViewModelMfc<FacturaRapida> {
        ...
        private void CalcularTotalDescuento() {
            ...
            if (totales > 0) {
                TotalDescuento = LibMath.RoundToNDecimals(totales * (Model.PorcentajeDescuento / 100), 2);
            } else {
                TotalDescuento = 0;
            }
        ...
        private void ActualizaTotalesDeFactura() {
            ...
            MontoGravableAlicuota1 = CalcularDescuento(DetailFacturaRapidaDetalle.MontoGravableGeneral(2), vFactorDescuento);
            MontoGravableAlicuota2 = CalcularDescuento(DetailFacturaRapidaDetalle.MontoGravableReducida(2), vFactorDescuento);
            MontoGravableAlicuota3 = CalcularDescuento(DetailFacturaRapidaDetalle.MontoGravableExtendida(2), vFactorDescuento);
        ...
        public void AgregarArticuloAlGridSiNecesario() {
            ...
            if (ConexionArticulo != null) {
                ...
                if (!vAcumularItemsEnRenglonesDeFactura || !vYaExiste) {
                    ...
                    if (vEsNecesarioUsarPrecioMe) {
                        vCambio = vUsaDivisaMonedaPredeterminada ? 1 : CambioMostrarTotalEnDivisas;
                        vNivelDePrecio = new clsLibSaw().ObtenerNivelDePrecio(ConsecutivoCompania, CodigoCliente);
                        ObtenerPrecioConIVAMEyPrecioSinIVAMESegunNivelDePrecio(vNivelDePrecio, ref vPrecioSinIva, ref vPrecioConIva);
                        vPrecioSinIva = LibMath.RoundToNDecimals(vPrecioSinIva * vCambio, _CantidadDeDecimales);
                        vPrecioConIva = LibMath.RoundToNDecimals(vPrecioConIva * vCambio, _CantidadDeDecimales);
                        if (FormaDeCalculoDePrecioRenglon == eFormaDeCalculoDePrecioRenglonFactura.APartirDelPrecioConIVA) {
                            vPrecioSinIva = CalcularPrecioSinIva(vPrecioConIva, (eTipoDeAlicuota)ConexionArticulo.AlicuotaIVA);
                        }
                        DetailFacturaRapidaDetalle.InsertRow(Articulo, Descripcion, Cantidad, vPrecioSinIva, vPrecioConIva, (eTipoDeAlicuota)ConexionArticulo.AlicuotaIVA, LibConvert.ToDec(ConexionArticulo.PorcentajeBaseImponible), ConexionArticulo.TipoDeArticulo, ConexionArticulo.TipoArticuloInv);
                    } else {
                        vPrecioConIva = ConexionArticulo.PrecioConIVA;
                        if (FormaDeCalculoDePrecioRenglon == eFormaDeCalculoDePrecioRenglonFactura.APartirDelPrecioConIVA) {
                            vPrecioSinIva = CalcularPrecioSinIva(vPrecioConIva, (eTipoDeAlicuota)ConexionArticulo.AlicuotaIVA);
                        } else {
        ...
        public bool AsignarTasaDeCambioDeMonedaDeCobroYParaMostrarTotales(bool valFacturaEnEspera = false) {
            ...
            bool vInsertarCambio = LibGlobalValues.Instance.GetAppMemInfo().GlobalValuesGetBool("Parametros", "UsaMonedaExtranjera") && EsNecesarioInsertarCambio();
            if (vInsertarCambio) {               
                ...
                CambioMostrarTotalEnDivisas = clsSawCambio.InsertaTasaDeCambioParaElDia(vMoneda, LibDate.Today(), vUsarLimiteMaximoParaIngresoDeTasaDeCambio, vMaximoLimitePermitidoParaLaTasaDeCambio, vElProgramaEstaEnModoAvanzado, vObtenerAutomaticamenteTasaDeCambioDelBCV);
                return CambioMostrarTotalEnDivisas > 0;
            } else {
                CambioMostrarTotalEnDivisas = 1;
                vResult = true;
            }
        ...
        decimal CalcularPrecioSinIva(decimal valPRecioConIva, eTipoDeAlicuota valTipoDeAlicuota) {
            decimal vResult = 0;
            ...
            return LibMath.RoundToNDecimals(valPRecioConIva / vResult, _CantidadDeDecimales);
        }

        private void ObtenerPrecioConIVAMEyPrecioSinIVAMESegunNivelDePrecio(int valNivelDePrecio, ref decimal refPrecioMESinIva, ref decimal refPrecioMEConIva) {
            switch (valNivelDePrecio) {
                case 1:
                    refPrecioMEConIva = ConexionArticulo.MePrecioConIva;
                    refPrecioMESinIva = ConexionArticulo.MePrecioSinIva;
                    break;
                case 2:
                    refPrecioMEConIva = ConexionArticulo.MePrecioConIva2;
                    refPrecioMESinIva = ConexionArticulo.MePrecioSinIva2;
                    break;
                case 3:
                    refPrecioMEConIva = ConexionArticulo.MePrecioConIva3;
                    refPrecioMESinIva = ConexionArticulo.MePrecioSinIva3;
                    break;
                case 4:
                    refPrecioMEConIva = ConexionArticulo.MePrecioConIva4;
                    refPrecioMESinIva = ConexionArticulo.MePrecioSinIva4;
                    break;
                default:
                    refPrecioMEConIva = ConexionArticulo.MePrecioConIva;
                    refPrecioMESinIva = ConexionArticulo.MePrecioSinIva;
        ...
        private decimal CalcularDescuento(decimal vMonto, decimal vDescuento) {
