...
namespace Galac.Adm.Uil.Banco.ViewModel {
    public class CuentaBancariaViewModel : LibInputViewModelMfc<CuentaBancaria> {
        ...
        private ValidationResult IsValidMoneda() {
            ...
            string vCodigoPais = LibGlobalValues.Instance.GetAppMemInfo().GlobalValuesGetString("RecordName", "CodigoPais");
            string vCodigoMonedaLocal = LibGlobalValues.Instance.GetAppMemInfo().GlobalValuesGetString("RecordName", "CodigoMonedaLocal");
            string vNombreMonedaLocal = LibGlobalValues.Instance.GetAppMemInfo().GlobalValuesGetString("RecordName", "NombreMonedaLocal");
            string vHoyCodigoMoneda = LibGlobalValues.Instance.GetAppMemInfo().GlobalValuesGetString("RecordName", "HoyCodigoMoneda");
        ...
        public bool IsVisibleITF {
            get {
                return ((LibGlobalValues.Instance.GetAppMemInfo().GlobalValuesGetBool("RecordName", "ManejaDebitoBancario")) ? !EsEcuador() : false);
            }
        }

        public bool IsVisibleITFCreditoBancario {
            get {
                bool result = LibGlobalValues.Instance.GetAppMemInfo().GlobalValuesGetBool("RecordName", "ManejaDebitoBancario") && !LibGalac.Aos.DefGen.LibDefGen.ProgramInfo.IsCountryVenezuela() && !EsEcuador();
                return result;
            }
        }
        ...
        private void AgregarCriteriaParaExcluirMonedasLocalesNoVigentesAlDiaActual(ref LibSearchCriteria vFixedCriteria) {
            ...
            if (vListaDeMonedaLocales != null) {
                foreach (MonedaLocalActual vMoneda in vListaDeMonedaLocales) {
                    if (vMoneda.CodigoMoneda != vMonedaLocalActual.GetHoyCodigoMoneda()) {
