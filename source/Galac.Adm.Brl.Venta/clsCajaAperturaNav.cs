...
namespace Galac.Adm.Brl.Venta {
    public partial class clsCajaAperturaNav:LibBaseNav<IList<CajaApertura>,IList<CajaApertura>>, ICajaAperturaPdn {
        ...
        bool ICajaAperturaPdn.TotalesMontosPorFormaDecobro(ref XElement refResult, int valconsecutivoCompania, int valConsecutivoCaja, DateTime valFechaApertura, string valHoraApertura, string valCodigoMonedaME) {
            ...
            vSql.AppendLine(insSql.IIF("FormaDelCobro.TipoDePago = " + insSql.EnumToSqlValue((int)eTipoDeFormaDePago.Deposito), "SUM(renglonCobroDeFactura.Monto)", "0", true) + " AS MontoDeposito, ");
            vSql.AppendLine(insSql.IIF("FormaDelCobro.TipoDePago = " + insSql.EnumToSqlValue((int)eTipoDeFormaDePago.Anticipo), "SUM(renglonCobroDeFactura.Monto)", "0", true) + " AS MontoAnticipo, ");
            vSql.AppendLine(insSql.IIF("FormaDelCobro.TipoDePago = " + insSql.EnumToSqlValue((int)eTipoDeFormaDePago.Cheque), "SUM(renglonCobroDeFactura.Monto)", "0", true) + " AS MontoCheque, ");
            vSql.AppendLine(insSql.IIF("FormaDelCobro.TipoDePago = " + insSql.EnumToSqlValue((int)eTipoDeFormaDePago.VueltoEfectivo), "(SUM(renglonCobroDeFactura.Monto) * -1)", "0", true) + " AS MontoVuelto, ");
            vSql.AppendLine(insSql.IIF("FormaDelCobro.TipoDePago = " + insSql.EnumToSqlValue((int)eTipoDeFormaDePago.VueltoC2P), "(SUM(renglonCobroDeFactura.Monto) * -1)", "0", true) + " AS MontoVueltoPM, ");
            ...
            vSql.AppendLine(" , 0 AS MontoEfectivoME, 0 AS MontoTarjetaME, 0 AS MontoDepositoME ,0 AS MontoAnticipoME, 0 AS MontoChequeME, 0 AS MontoVueltoME, 0 AS MontoZelle, 0 AS MontoCreditoElectronico");
            ...
            if (!LibString.S1IsEqualToS2(valCodigoMonedaME, vCodigoMonedaLocal)) {
                ...
                vSql.AppendLine(insSql.IIF("FormaDelCobro.TipoDePago = " + insSql.EnumToSqlValue((int)eTipoDeFormaDePago.Anticipo), "SUM(renglonCobroDeFactura.Monto)", "0", true) + " AS MontoAnticipoME, ");
                vSql.AppendLine(insSql.IIF("FormaDelCobro.TipoDePago = " + insSql.EnumToSqlValue((int)eTipoDeFormaDePago.Cheque), "SUM(renglonCobroDeFactura.Monto)", "0", true) + " AS MontoChequeME, ");
                vSql.AppendLine(insSql.IIF("FormaDelCobro.TipoDePago = " + insSql.EnumToSqlValue((int)eTipoDeFormaDePago.VueltoEfectivo), "(SUM(renglonCobroDeFactura.Monto) * -1)", "0", true) + " AS MontoVueltoME, ");
                vSql.AppendLine(insSql.IIF("FormaDelCobro.TipoDePago = " + insSql.EnumToSqlValue((int)eTipoDeFormaDePago.Zelle), "SUM(renglonCobroDeFactura.Monto)", "0", true) + " AS MontoZelle, ");
                vSql.AppendLine(insSql.IIF("FormaDelCobro.TipoDePago = " + insSql.EnumToSqlValue((int)eTipoDeFormaDePago.CreditoElectronico), "SUM(renglonCobroDeFactura.Monto)", "0", true) + " AS MontoCreditoElectronico ");
                vSql.AppendLine("  FROM renglonCobroDeFactura");
                vSql.AppendLine("  INNER JOIN  FormaDelCobro ON");
                vSql.AppendLine("  renglonCobroDeFactura.CodigoFormaDelCobro = FormaDelCobro.Codigo");
                vSql.AppendLine("  INNER JOIN factura  ON");
