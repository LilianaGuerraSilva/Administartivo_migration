...
namespace Galac.Adm.Brl.Venta {
    public partial class clsCajaAperturaNav:LibBaseNav<IList<CajaApertura>,IList<CajaApertura>>, ICajaAperturaPdn {
        ...
        bool ICajaAperturaPdn.TotalesMontosPorFormaDecobro(ref XElement refResult, int valconsecutivoCompania, int valConsecutivoCaja, DateTime valFechaApertura, string valHoraApertura, string valCodigoMonedaME) {
            ...
            vSql.AppendLine(insSql.IIF("FormaDelCobro.TipoDePago = " + insSql.EnumToSqlValue((int)eFormaDeCobro.VueltoPM), "(SUM(renglonCobroDeFactura.Monto) * -1)", "0", true) + " AS MontoVueltoPM, ");
            vSql.AppendLine(insSql.IIF("FormaDelCobro.TipoDePago = " + insSql.EnumToSqlValue((int)eFormaDeCobro.C2P), "SUM(renglonCobroDeFactura.Monto)", "0", true) + " AS MontoC2P, ");
            vSql.AppendLine(insSql.IIF("FormaDelCobro.TipoDePago = " + insSql.EnumToSqlValue((int)eFormaDeCobro.TarjetaMS), "SUM(renglonCobroDeFactura.Monto)", "0", true) + " AS MontoTarjetaMS, ");
            vSql.AppendLine(insSql.IIF("FormaDelCobro.TipoDePago = " + insSql.EnumToSqlValue((int)eFormaDeCobro.PagoMovil), "SUM(renglonCobroDeFactura.Monto)", "0", true) + " AS MontoPagoMovil, ");
            vSql.AppendLine(insSql.IIF("FormaDelCobro.TipoDePago = " + insSql.EnumToSqlValue((int)eFormaDeCobro.TransferenciaMS), "SUM(renglonCobroDeFactura.Monto)", "0", true) + " AS MontoTransferenciaMS, ");
            vSql.AppendLine(insSql.IIF("FormaDelCobro.TipoDePago = " + insSql.EnumToSqlValue((int)eFormaDeCobro.DepositoMS), "SUM(renglonCobroDeFactura.Monto)", "0", true) + " AS MontoDepositoMS");
            ...
            if (!LibString.S1IsEqualToS2(valCodigoMonedaME, vCodigoMonedaLocal)) {
                ...
                vSql.AppendLine(insSql.IIF("FormaDelCobro.TipoDePago = " + insSql.EnumToSqlValue((int)eFormaDeCobro.Zelle), "SUM(renglonCobroDeFactura.Monto)", "0", true) + " AS MontoZelle, ");
                vSql.AppendLine(insSql.IIF("FormaDelCobro.TipoDePago = " + insSql.EnumToSqlValue((int)eFormaDeCobro.CreditoElectronico), "SUM(renglonCobroDeFactura.Monto)", "0", true) + " AS MontoCreditoElectronico ");
                vSql.AppendLine("  FROM renglonCobroDeFactura");
                vSql.AppendLine("  INNER JOIN  FormaDelCobro ON");
                vSql.AppendLine("  renglonCobroDeFactura.CodigoFormaDelCobro = FormaDelCobro.Codigo");
                vSql.AppendLine("  INNER JOIN factura  ON");
